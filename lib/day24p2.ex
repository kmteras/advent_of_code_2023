defmodule Day24P2 do
  ...
  defp check_intersections(lines, s, e) do
    # Try to implement:
    # https://math.stackexchange.com/questions/2536794/line-intersecting-3-other-lines
    # https://math.stackexchange.com/a/2925405

    lines
    |> Enum.flat_map(fn {{x1, y1, _}, {vx1, vy1, _}} = l1 ->
      Enum.flat_map(lines, fn {{x2, y2, _}, {vx2, vy2, _}} = l2 ->
        if l1 != l2 do
          #          IO.inspect((vx1 * vx2 - vy1 * vx2))
          det = vx1 * vy2 - vy1 * vx2

          if det != 0 do
            u = (y1 * vx2 + vy2 * x2 - y2 * vx2 - vy2 * x1) / det
            #        u = (y1 * vx2 + vy2 * x2 - y2 * vx2 - vy2 * x1) / (vx1 * vx2 - vy1 * vx2)
            v = (x1 + vx1 * u - x2) / vx2
